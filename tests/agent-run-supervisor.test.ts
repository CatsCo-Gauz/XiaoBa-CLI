...
function fixture(options: {
  ...
  checks?: Array<StructuredGoalCheck | Error | Promise<StructuredGoalCheck>>;
  filePath?: string;
  onTurnStarted?: () => void;
  onCheckStarted?: () => void;
} = {}) {
...
describe('AgentRunSupervisor', () => {
  ...
  test('consumes every input queued during one active turn in arrival order', async () => {
    let release!: (value: string) => void;
    let entered!: () => void;
    const started = new Promise<void>(resolve => { entered = resolve; });
    const pending = new Promise<string>(resolve => { release = resolve; });
    const fx = fixture({
      finals: [pending, 'combined input final'],
      onTurnStarted: entered,
      checks: [
        { decision: 'complete', summary: 'first turn complete' },
        { decision: 'complete', summary: 'queued inputs complete' },
      ],
    });
    const active = start(fx.supervisor);
    await started;
    const firstInput = fx.supervisor.send('run-1', 'FIRST-INPUT');
    const secondInput = fx.supervisor.send('run-1', 'SECOND-INPUT');
    release('first final');
    await Promise.all([active, firstInput, secondInput]);

    assert.equal(fx.prompts[1], 'FIRST-INPUT\nSECOND-INPUT');
    assert.equal(fx.supervisor.show('run-1').status, 'completed');
  });
  ...
  test('keeps cancellation terminal when Goal Check completes late', async () => {
    let release!: (value: StructuredGoalCheck) => void;
    let checking!: () => void;
    const startedChecking = new Promise<void>(resolve => { checking = resolve; });
    const pendingCheck = new Promise<StructuredGoalCheck>(resolve => { release = resolve; });
    const fx = fixture({
      finals: ['turn final'],
      checks: [pendingCheck],
      onCheckStarted: checking,
    });
    const active = start(fx.supervisor);
    await startedChecking;
    assert.equal(fx.supervisor.cancel('run-1').status, 'cancelled');
    release({ decision: 'complete', summary: 'late complete' });
    const result = await active;
