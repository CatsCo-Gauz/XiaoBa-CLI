...
import { ensureDirectHttpsHost } from "./direct-https-runtime.mjs";
import {
...
async function main() {
  ...
  try {
    ...
    if (useDirectRuntime) {
      const directHttps = await ensureDirectHttpsHost();
      args = mergePublishProfile(rawArgs, { directHost: directHttps.runtime });
    ...
    if (args.provider === "catsco-central") {
      ...
      if (!["static-dir", "direct-ip", "direct-https"].includes(args.provider)) {
        throw new Error(
          "publish-html-directory requires direct-https, direct-ip, static-dir, or an explicit catsco-central compatibility profile"
        );
      }
      ...
      const localQaBaseUrl = ["direct-ip", "direct-https"].includes(args.provider)
        ? requiredHttpUrl(args["local-qa-base-url"], "profile.localQaBaseUrl").replace(/\/+$/, "")
        : publicBaseUrl;
      const qaBaseUrl = ["direct-ip", "direct-https"].includes(args.provider)
