...
export class AIServiceGoalChecker implements AgentRunGoalChecker {
  ...
  async check(input: GoalCheckInput): Promise<StructuredGoalCheck> {
    ...
    if (calls.length === 0) throw new Error('goal_check_missing_structured_call');
    if (calls.length !== 1 || calls[0].function.name !== GOAL_CHECK_TOOL.name) {
      throw new Error('goal_check_invalid_structured_calls');
    }
    return parseStructuredGoalCheck(calls[0].function.arguments);
  }
}
...
export function parseStructuredGoalCheck(raw: string): StructuredGoalCheck {
  ...
  if (decision === 'continue' && blocker) throw new Error('goal_check_continue_has_blocker');
  if (decision === 'blocked' && !blocker) throw new Error('goal_check_blocked_requires_blocker');
  if (decision === 'blocked' && nextAction) throw new Error('goal_check_blocked_has_next_action');
  if (decision !== 'complete' && !stopCondition) throw new Error('goal_check_incomplete_requires_stop_condition');
  if (decision === 'complete' && (nextAction || blocker || stopCondition || nextWakeAt)) {
    throw new Error('goal_check_complete_has_continuation');
  }
  if (nextWakeAt && !isCanonicalTimestamp(nextWakeAt)) throw new Error('goal_check_invalid_next_wake_at');
  return {
    decision,
    summary,
