...
export class AgentRunSupervisor {
  ...
  async send(runId: string, message: string): Promise<AgentRunRecord> {
    ...
    this.updateWithConflictRetry(runId, mutable => {
      this.assertRunnable(mutable);
  ...
  cancel(runId: string): AgentRunRecord {
    return this.updateWithConflictRetry(runId, mutable => {
      if (mutable.status === 'completed') throw new Error('Cannot cancel a completed Agent Run');
      if (mutable.status === 'cancelled') return;
  ...
  private runOwnedLoop(runId: string, wakeReason?: string): Promise<AgentRunRecord> {
    ...
    if (existing) {
      return existing.then(run => run.status === 'queued' ? this.runOwnedLoop(runId) : run);
    }
    const operation = withExclusiveFileLockAsync(path.join(this.ownerLockRoot, `${safeFileName(runId)}.lock`), async ({ contended }) => {
      this.store.refresh();
      if (contended) {
        const run = this.requireRun(runId);
        return run.status === 'queued' ? this.performLoop(runId) : run;
      }
      if (wakeReason) {
  ...
  private async performLoop(runId: string): Promise<AgentRunRecord> {
    ...
    let pendingInputs = unconsumedInputs(run);
    let nextPrompt = pendingInputs.length
      ? joinInputs(pendingInputs)
    ...
    while (true) {
      ...
      this.updateWithConflictRetry(runId, mutable => {
        ...
        mutable.events.push(this.dataEvent('iteration_started', {
          iteration,
          consumedInputEventIds: pendingInputs.map(event => event.eventId).filter(Boolean),
        }));
      });
      this.store.refresh();
      ...
      let queuedInputs: AgentRunEvent[] = [];
      run = this.updateWithConflictRetry(runId, mutable => {
        if (mutable.status === 'cancelled') return;
        queuedInputs = unconsumedInputs(mutable);
        ...
        mutable.status = queuedInputs.length
          ? 'queued'
      ...
      if (queuedInputs.length) {
        ...
        nextPrompt = joinInputs(queuedInputs);
        continue;
      }
  ...
  private updateWithConflictRetry(
    runId: string,
    update: (record: AgentRunRecord) => AgentRunRecord | void,
  ): AgentRunRecord {
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        return this.store.update(runId, update);
      } catch (error) {
        if (!(error instanceof Error) || !error.message.startsWith('agent run changed concurrently:') || attempt === 3) {
          throw error;
        }
      }
    }
    throw new Error(`agent run update retry exhausted: ${runId}`);
  }
...
function unconsumedInputs(run: AgentRunRecord): AgentRunEvent[] {
  const consumed = new Set<string>();
  for (const event of run.events) {
    if (event.type !== 'iteration_started') continue;
    for (const eventId of parseConsumedInputEventIds(event.summary)) consumed.add(eventId);
  }
...
function joinInputs(events: AgentRunEvent[]): string {
  return events
    .map(event => parseTextEvent(event.summary))
    .filter((item): item is string => Boolean(item))
    .join('\n');
...
function parseConsumedInputEventIds(summary: string): string[] {
  try {
    const value = JSON.parse(summary) as { consumedInputEventIds?: unknown };
    return Array.isArray(value.consumedInputEventIds)
      ? value.consumedInputEventIds.filter((item): item is string => typeof item === 'string' && !!item)
      : [];
  } catch {
    return [];
  }
