...
export class ReviewAdapter implements ReviewRuntimeToolController {
  ...
  async triggerFinding(input: TriggerFindingInput): Promise<ReviewRunRecord> {
    ...
    if (!run) {
      ...
      run = this.store.create({
        ...
        goal: input.goal?.trim() || defaultGoal(findingId),
        envelopePath,
        ...
        events: [{
          ...
          at: now,
          actor: input.actor,
          summary: `Review Run created for ${findingId}`,
        }],
      });
    } else if (path.resolve(run.envelopePath) !== envelopePath) {
  ...
  async wakeRun(runId: string, reason: string, actor: string): Promise<ReviewRunRecord> {
    if (this.activeWakes.has(runId)) return this.requireRun(runId);
    this.activeWakes.add(runId);
    try {
  ...
  async proposeTask(runId: string, sessionKey: string | undefined, spec: ReviewTaskSpec): Promise<ReviewTaskRecord> {
    ...
    const duplicate = Object.values(this.requireRun(runId).tasks)
      .find(task => task.idempotencyKey === idempotencyKey);
    if (duplicate) return duplicate;
    const taskId = `task-${this.idFactory()}`;
    let created!: ReviewTaskRecord;
...
function defaultGoal(findingId: string): string {
  return `Build a complete Evidence Envelope for ${findingId} and decide whether to recommend Issue or Close; otherwise record the concrete blocker, next evidence action, and stop condition.`;
}
